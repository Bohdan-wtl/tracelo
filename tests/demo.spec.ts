...
const linksToTest = linksGroups[group];
...
test.describe(`Stripe Payment Test - Group ${group}`, () => {
  for (const link of linksToTest) {
    for (const [countryCode, card] of Object.entries(cards)) {
      test(`Payment test for link: ${link} with card for country: ${countryCode}`, async ({ page }) => {
        console.log(`Running tests for Group ${process.env.GROUP || "1"}`);
        ...
        console.log(`Transaction passed for ${link} with card ${card.number} using email ${fake_email}`);
      });
    }
  }
});
